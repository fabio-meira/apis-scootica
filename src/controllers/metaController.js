    ...
    try {
        ...
        if (metaExists) {
            return res.status(400).json({
                error: "Meta já cadastrado no sistema"
            });
        }
        
        // Adiciona o cpf como idMeta no objeto metaData
        metaData.idEmpresa = idEmpresa;

        const meta = await Meta.create(metaData);

        res.status(201).json({ message: 'Meta cadastrado com sucesso', meta });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Erro ao cadastrar meta', error });
    }
}
...
async function getVendedor(req, res) {
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'Erro ao buscar meta', error });
    }
}

// Função para consultar metas e suas vendas por um período específico ou todos os pedidos
async function getVendasMeta(req, res) {
    try {
...
async function putVendedor(req, res) {
    try {
        ...
        const [updated] = await Vendedor.update(metaData, {
            where: { 
                idEmpresa: idEmpresa,
                id: id
            }
